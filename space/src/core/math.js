...
export function hash01(n) {
  // 32비트 정수 연산으로 계산한다. 부동소수 곱으로 하면 n 이 커질 때
  // 2^53 을 넘어 하위 비트가 사라지고 해시가 상수로 붕괴한다.
  let x = n | 0;
  x = (x << 13) ^ x;
  const t = (Math.imul(Math.imul(x, x), 15731) + 789221) | 0;
  const v = (Math.imul(x, t) + 1376312589) | 0;
  return (v & 0x7fffffff) / 2147483647;
}
